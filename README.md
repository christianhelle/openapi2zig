...
The simplest way is to let Zig write the entry and compute the hash for you:

```bash
zig fetch --save https://github.com/christianhelle/openapi2zig/archive/refs/tags/v0.5.2.tar.gz
```

That produces a `build.zig.zon` along these lines. Note that `.name` is an enum literal, not a string, and that `.fingerprint` is generated once by the Zig toolchain and then never edited:

```zig
.{
    .name = .my_project,
    .version = "0.1.0",
    .fingerprint = 0x0, // generated by `zig build`; do not edit
