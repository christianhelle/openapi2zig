...
pub const uploadFileOptions = struct {
    petId: i64,
    additionalMetadata: ?[]const u8 = null,
};

pub fn uploadFile(client: *Client, options: uploadFileOptions, requestBody: []const u8) !Owned(ApiResponse) {
    var result = try uploadFileResult(client, options, requestBody);
    switch (result) {
        .ok => |ok| return ok,
        .api_error => |*err| {
            err.deinit();
...
pub fn uploadFileRaw(client: *Client, options: uploadFileOptions, requestBody: []const u8) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
    try uri_buf.writer.print("{s}/pet/{f}/uploadImage", .{ client.base_url, pathComponent(options.petId) });
    var first_query = true;
    if (options.additionalMetadata) |value| {
...
pub fn uploadFileResult(client: *Client, options: uploadFileOptions, requestBody: []const u8) !ApiResult(ApiResponse) {
    return parseRawResponse(ApiResponse, try uploadFileRaw(client, options, requestBody));
}
...
//
pub const getPetByIdOptions = struct {
    petId: i64,
};

pub fn getPetById(client: *Client, options: getPetByIdOptions) !Owned(Pet) {
    var result = try getPetByIdResult(client, options);
    switch (result) {
        .ok => |ok| return ok,
        .api_error => |*err| {
            err.deinit();
...
pub fn getPetByIdRaw(client: *Client, options: getPetByIdOptions) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
    try uri_buf.writer.print("{s}/pet/{f}", .{ client.base_url, pathComponent(options.petId) });
...
pub fn getPetByIdResult(client: *Client, options: getPetByIdOptions) !ApiResult(Pet) {
    return parseRawResponse(Pet, try getPetByIdRaw(client, options));
}
...
pub fn updatePetWithForm(client: *Client, options: updatePetWithFormOptions) !void {
    var raw = try updatePetWithFormRaw(client, options);
    defer raw.deinit();
    if (raw.status.class() != .success) return error.ResponseError;
}

pub fn updatePetWithFormRaw(client: *Client, options: updatePetWithFormOptions) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
    try uri_buf.writer.print("{s}/pet/{f}", .{ client.base_url, pathComponent(options.petId) });
...
pub fn deletePet(client: *Client, options: deletePetOptions) !void {
    var raw = try deletePetRaw(client, options);
    defer raw.deinit();
    if (raw.status.class() != .success) return error.ResponseError;
}

pub fn deletePetRaw(client: *Client, options: deletePetOptions) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
    try uri_buf.writer.print("{s}/pet/{f}", .{ client.base_url, pathComponent(options.petId) });
...
//
pub const findPetsByTagsOptions = struct {
    tags: ?[]const u8 = null,
};

pub fn findPetsByTags(client: *Client, options: findPetsByTagsOptions) !Owned([]const std.json.Value) {
    var result = try findPetsByTagsResult(client, options);
    switch (result) {
        .ok => |ok| return ok,
        .api_error => |*err| {
            err.deinit();
...
pub fn findPetsByTagsRaw(client: *Client, options: findPetsByTagsOptions) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
...
pub fn findPetsByTagsResult(client: *Client, options: findPetsByTagsOptions) !ApiResult([]const std.json.Value) {
    return parseRawResponse([]const std.json.Value, try findPetsByTagsRaw(client, options));
}
...
pub fn loginUser(client: *Client, options: loginUserOptions) !Owned([]const u8) {
    var result = try loginUserResult(client, options);
    switch (result) {
        .ok => |ok| return ok,
        .api_error => |*err| {
            err.deinit();
...
pub fn loginUserRaw(client: *Client, options: loginUserOptions) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
...
pub fn loginUserResult(client: *Client, options: loginUserOptions) !ApiResult([]const u8) {
    return parseRawResponse([]const u8, try loginUserRaw(client, options));
}
...
//
pub const findPetsByStatusOptions = struct {
    status: ?[]const u8 = null,
};

pub fn findPetsByStatus(client: *Client, options: findPetsByStatusOptions) !Owned([]const std.json.Value) {
    var result = try findPetsByStatusResult(client, options);
    switch (result) {
        .ok => |ok| return ok,
        .api_error => |*err| {
            err.deinit();
...
pub fn findPetsByStatusRaw(client: *Client, options: findPetsByStatusOptions) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
...
pub fn findPetsByStatusResult(client: *Client, options: findPetsByStatusOptions) !ApiResult([]const std.json.Value) {
    return parseRawResponse([]const std.json.Value, try findPetsByStatusRaw(client, options));
}
...
pub fn getUserByName(client: *Client, options: getUserByNameOptions) !Owned(User) {
    var result = try getUserByNameResult(client, options);
    switch (result) {
        .ok => |ok| return ok,
        .api_error => |*err| {
            err.deinit();
...
pub fn getUserByNameRaw(client: *Client, options: getUserByNameOptions) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
    try uri_buf.writer.print("{s}/user/{f}", .{ client.base_url, pathComponent(options.username) });
    const payload: ?[]const u8 = null;
...
pub fn getUserByNameResult(client: *Client, options: getUserByNameOptions) !ApiResult(User) {
    return parseRawResponse(User, try getUserByNameRaw(client, options));
}
...
//
pub const updateUserOptions = struct {
    username: []const u8,
};

pub fn updateUser(client: *Client, options: updateUserOptions, requestBody: User) !void {
    var raw = try updateUserRaw(client, options, requestBody);
    defer raw.deinit();
    if (raw.status.class() != .success) return error.ResponseError;
}

pub fn updateUserRaw(client: *Client, options: updateUserOptions, requestBody: User) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
    try uri_buf.writer.print("{s}/user/{f}", .{ client.base_url, pathComponent(options.username) });
...
// This can only be done by the logged in user.
//
pub const deleteUserOptions = struct {
    username: []const u8,
};

pub fn deleteUser(client: *Client, options: deleteUserOptions) !void {
    var raw = try deleteUserRaw(client, options);
    defer raw.deinit();
    if (raw.status.class() != .success) return error.ResponseError;
}

pub fn deleteUserRaw(client: *Client, options: deleteUserOptions) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
    try uri_buf.writer.print("{s}/user/{f}", .{ client.base_url, pathComponent(options.username) });
...
//
pub const getOrderByIdOptions = struct {
    orderId: i64,
};

pub fn getOrderById(client: *Client, options: getOrderByIdOptions) !Owned(Order) {
    var result = try getOrderByIdResult(client, options);
    switch (result) {
        .ok => |ok| return ok,
        .api_error => |*err| {
            err.deinit();
...
pub fn getOrderByIdRaw(client: *Client, options: getOrderByIdOptions) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
    try uri_buf.writer.print("{s}/store/order/{f}", .{ client.base_url, pathComponent(options.orderId) });
    const payload: ?[]const u8 = null;
...
pub fn getOrderByIdResult(client: *Client, options: getOrderByIdOptions) !ApiResult(Order) {
    return parseRawResponse(Order, try getOrderByIdRaw(client, options));
}
...
//
pub const deleteOrderOptions = struct {
    orderId: i64,
};

pub fn deleteOrder(client: *Client, options: deleteOrderOptions) !void {
    var raw = try deleteOrderRaw(client, options);
    defer raw.deinit();
    if (raw.status.class() != .success) return error.ResponseError;
}

pub fn deleteOrderRaw(client: *Client, options: deleteOrderOptions) !RawResponse {
    const allocator = client.allocator;
    var uri_buf: std.Io.Writer.Allocating = .init(allocator);
    defer uri_buf.deinit();
    try uri_buf.writer.print("{s}/store/order/{f}", .{ client.base_url, pathComponent(options.orderId) });
