...
fn parseOpenApi32Schema(allocator: std.mem.Allocator, schema_json: []const u8) !models.v32.Schema {
    var parsed_json = try std.json.parseFromSlice(std.json.Value, allocator, schema_json, .{});
    defer parsed_json.deinit();
    return try models.v32.Schema.parseFromJson(allocator, parsed_json.value);
}
...
test "v3.2 schema parses integer numeric constraints" {
    var gpa = test_utils.createTestAllocator();
    const allocator = gpa.allocator();
    var schema = try parseOpenApi32Schema(allocator,
        \\{
        \\  "type": "integer",
        \\  "multipleOf": 5,
        \\  "minimum": 1,
