...
// Comprehensive test that validates all OpenAPI v3.0 JSON specifications can be converted to UnifiedDocument
...
// Comprehensive test that validates all Swagger v2.0 JSON specifications can be converted to UnifiedDocument
...
// Test that validates both v2.0 and v3.0 conversions work and produce valid UnifiedDocuments
test "unified conversion compatibility between Swagger v2.0 and OpenAPI v3.0" {
    var gpa = test_utils.createTestAllocator();
    const allocator = gpa.allocator();

    // Test with similar petstore examples from both versions
    ...
    // Convert both to UnifiedDocument
    ...
    // Both should have the same title (Swagger Petstore)
